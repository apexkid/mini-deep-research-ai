...
class Orchestrator:
    ...
    async def _research_queries(
        self, queries: List[str], existing_findings: List[Finding]
    ) -> List[Finding]:
        ...
        for search_query in queries:
            ...
            self.search_count += 1
            console.print(
                f"  [dim]Searching ({self.search_count}/{self.config.max_searches}): {search_query}[/dim]"
            )
            ...
            for page in pages:
                ...
                # Use the specific search query for extraction to ensure we target the specific information
                extractor_findings = await self.extractor.extract_findings(
                    search_query, page
                )
    ...
    async def run(self, query: str):
        ...
        langfuse.update_current_trace(
            name="deep_research_run", input=query, tags=[self.config.gemini_model]
        )
        ...
        for i, sq in enumerate(sorted_sqs, 1):
            ...
            sq_findings = await self._research_queries(sq.queries, all_findings)

            # --- GAP ANALYSIS & FOLLOW-UP LOOP ---
            current_depth = 1
            while current_depth < self.config.max_depth and not self.budget_exhausted:
                console.print(
                    f"  [bold yellow][GAP][/bold yellow] Analyzing results (depth {current_depth}) for: {sq.question}"
                )
                gap_analysis = await self.gap_analyzer.analyze_gaps(
                    sq.question, sq_findings
                )

                if gap_analysis.is_satisfied or not gap_analysis.follow_up_queries:
                    console.print(
                        f"  [bold green][GAP][/bold green] Sub-question satisfied."
                    )
                    break

                console.print(
                    f"  [bold yellow][GAP][/bold yellow] Missing: {gap_analysis.explanation}"
                )
                console.print(
                    f"  [bold yellow][GAP][/bold yellow] Running follow-up research with {len(gap_analysis.follow_up_queries)} queries..."
                )

                follow_up_findings = await self._research_queries(
                    gap_analysis.follow_up_queries, all_findings + sq_findings
                )
                sq_findings.extend(follow_up_findings)
                console.print(
                    f"  [cyan]Follow-up complete: {len(follow_up_findings)} new findings.[/cyan]"
                )
                current_depth += 1

            console.print(
                f"  [cyan]Sub-question complete: {len(sq_findings)} total findings.[/cyan]"
            )
        ...
        if self.budget_exhausted:
            console.print(
                "[bold red][BUDGET] Note: Research was cut short due to budget limits.[/bold red]"
            )

        # --- SYNTHESIS STEP ---
        console.print(f"[bold green]Synthesizing final report...[/bold green]")
        report_md = await self.synthesizer.synthesize_report(
            query, all_findings, budget_exhausted=self.budget_exhausted
        )
